...
- **CI 健康再次验证（commit `03f45c9`，P1-017）**：R10 闭环后 R9 / R10 两次 docs commit 在 CI 干净 Linux runner 上 pre-existing 红条回潮（`gh run list` 显示 `#30844146217` + `#30839992404` 都 `failure`）。**根因**：`tests/unit/test_gitignore_coverage.py` 旧版 12 条历史工件测试用 `git status --ignored --porcelain` 解析 `!!` 前缀 —— `git status --ignored` **只对物理存在的路径报告 ignored**，CI runner checkout 后没把 `data/eval_baseline_*.json` 等本地调试产物 copy 过去 → 12 条 assert 全 fail。**修复**：12 参化路径改 `subprocess.run(['git', '-c', 'core.quotepath=false', 'check-ignore', '-v', path])` 直接探测 .gitignore 规则覆盖（rc=0 = 被忽略，**不读文件系统**）；删除 `_git_status_ignored_files` / `_git_status_untracked_files` helper + `test_no_historical_artifacts_remain_untracked`（同样依赖物理存在）；新增 1 条组合守卫 `test_check_ignore_does_not_depend_on_physical_existence` 防止回潮。**路径调整**：原 plan 的 `debug_cached_response.py` 和 `services/_text_utils.py` 已在 P1-008（commit `4380c55`）入仓，`git check-ignore` 对 tracked 文件永远返 1 — 换成 `data/poll_streamlit.ps1` + `docs/portfolio.md`。**经 R10 docs 后两次 push 复测未回潮**：tests `#30848039867` 1m4s + secret-scan `#30848039710` 19s + docker-build `#30848039594` 33s 全 success。**P1-017 真正闭环**——CI 健康从"已修复"升级为"已修复 + 复测不回潮"。
