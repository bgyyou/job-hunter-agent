...
- [x] **P0-3**: §6 验收最后 1 条 — 3-5 个真实用户试用 + 反馈收集（招募方式 / 反馈表设计 / 数据收集表 / 反馈汇总成 round-3 收口报告）
  - ✅ AI 交付物：`docs/round3_user_trial.md`（招募话术 + 8 题反馈表 + JSONL schema + 收口模板）+ `scripts/aggregate_round3_feedback.py`（自动汇总 + Markdown 报告）+ 9 条单测全过；`.gitignore` 加 `data/round3_feedback.jsonl` 保护用户数据
  - ⏳ 用户责任：招募 3-5 个朋友 → 跑全流程 → 收集 JSONL → 跑汇总脚本 → 填到 `§8.1 round-3 收口报告`

**优先级 P1**（应该做）：

- [x] **P1-1**: T-extra-1 闭环 — Step 2 表单加"重置草稿"按钮（清空 v3 + legacy state）
